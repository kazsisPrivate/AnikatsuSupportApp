...
public class PstInfoSearcher extends DBOperator {
    ...
    private final int MBRAREA_HEIGHT = 150; // スクロールパネルの高さ
    ...
    private final int MBRNUMRDBT_Y = 350; // "部員番号"というボタンの左上端のy座標
    ...
    private final int HNMLB_Y = 350; // "HN"というラベルの左上端のy座標
    ...
    private JRadioButton year_rdbutton; // 入力フィールドの左に置く"発表年"というボタン
    private final int YRRDBT_WIDTH = RDBT_FONT_SIZE * 4; // "発表年"というボタンの幅
    private final int YRRDBT_HEIGHT = RDBT_FONT_SIZE; // "発表年"というボタンの高さ
    private final int YRRDBT_X = 100; // "発表年"というボタンの左上端のx座標
    private final int YRRDBT_Y = 420; // "発表年"というボタンの左上端のy座標
    private JTextField year_field; // 発表年を入力するテキストフィールド
    private final int YRFD_WIDTH = RDBT_FONT_SIZE * 3; // 発表年を入力するテキストフィールドの幅
    private final int YRFD_HEIGHT = RDBT_FONT_SIZE; // 発表年を入力するテキストフィールドの高さ
    private final int YRFD_X = YRRDBT_X + YRRDBT_WIDTH + RDBT_FONT_SIZE/3; // 発表年を表示するテキストフィールドの左上端のx座標
    private final int YRFD_Y = YRRDBT_Y + RDBT_FONT_SIZE/10; // 発表年を入力するテキストフィールドの左上端のy座標

    private JRadioButton title_rdbutton; // 入力フィールドの左に置く"タイトル"というボタン
    private final int TTLRDBT_WIDTH = RDBT_FONT_SIZE * 5; // "タイトル"というボタンの幅
    private final int TTLRDBT_HEIGHT = RDBT_FONT_SIZE; // "タイトル"というボタンの高さ
    private final int TTLRDBT_X = 350; // "タイトル"というボタンの左上端のx座標
    private final int TTLRDBT_Y = 420; // "タイトル"というボタンの左上端のy座標
    private JTextField title_field; // タイトルを入力するテキストフィールド
    private final int TTLFD_WIDTH = RDBT_FONT_SIZE * 9; // タイトルを入力するテキストフィールドの幅
    private final int TTLFD_HEIGHT = RDBT_FONT_SIZE; // タイトルを入力するテキストフィールドの高さ
    private final int TTLFD_X = TTLRDBT_X + TTLRDBT_WIDTH + RDBT_FONT_SIZE/3; // タイトルを入力するテキストフィールドの左上端のx座標
    private final int TTLFD_Y = TTLRDBT_Y + RDBT_FONT_SIZE/10; // タイトルを入力するテキストフィールドの左上端のy座標

    private JRadioButton url_rdbutton; // 入力フィールドの左に置く"URL"というボタン
    private final int URLRDBT_WIDTH = RDBT_FONT_SIZE * 3; // "URL"というボタンの幅
    private final int URLRDBT_HEIGHT = RDBT_FONT_SIZE; // "URL"というボタンの高さ
    private final int URLRDBT_X = 100; // "URL"というボタンの左上端のx座標
    private final int URLRDBT_Y = 490; // "URL"というボタンの左上端のy座標
    private JTextField url_field; // URLを入力するテキストフィールド
    private final int URLFD_WIDTH = RDBT_FONT_SIZE * 11; // URLを入力するテキストフィールドの幅
    private final int URLFD_HEIGHT = RDBT_FONT_SIZE; // URLを入力するテキストフィールドの高さ
    private final int URLFD_X = URLRDBT_X + URLRDBT_WIDTH + RDBT_FONT_SIZE/3; // URLを入力するテキストフィールドの左上端のx座標
    private final int URLFD_Y = URLRDBT_Y + RDBT_FONT_SIZE/10; // URLを入力するテキストフィールドの左上端のy座標
    ...
    private final int RES_AREA_Y = 660; // スクロールパネルの左上端のy座標
    ...
    public PstInfoSearcher(final ButtonResponse br, final BasicFunction bf) {
        ...
        // キャラクターに最初の説明を話させる
        bc_func.charaSpeak("検索方式を選択して、検索したい研究発表情報を入力し", "命令1");
        ...
        sch_icon = new ImageIcon("image/component/home/anikatsudb/presentation/searcher/icon/icon.png");
        ...
        // "発表年"のラベルを作る
        year_rdbutton = new JRadioButton("発表年");
        // フォントを設定
        ...
        // "タイトル"のラベルを作る
        title_rdbutton = new JRadioButton("タイトル");
        // フォントを設定
        title_rdbutton.setFont(RDBT_FONT);
        // 位置設定
        title_rdbutton.setBounds(TTLRDBT_X, TTLRDBT_Y, TTLRDBT_WIDTH, TTLRDBT_HEIGHT);
        // ボタンが選択されている状態にする
        title_rdbutton.setSelected(true);
        // ボタンの表示設定
        title_rdbutton.setContentAreaFilled(false);
        title_rdbutton.setBorderPainted(false);
        title_rdbutton.setFocusPainted(false);
        // ボタンを押した際の処理
        title_rdbutton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                System.out.println("no_button pressed");
                if (title_rdbutton.isSelected()) { // 選択されているとき
                    // 入力フィールドに書き込みを行える状態にする
        ...
        // "URL"のラベルを作る
        url_rdbutton = new JRadioButton("URL");
        // フォントを設定
        url_rdbutton.setFont(RDBT_FONT);
        // 位置設定
        url_rdbutton.setBounds(URLRDBT_X, URLRDBT_Y, URLRDBT_WIDTH, URLRDBT_HEIGHT);
        // ボタンが選択されている状態にする
        url_rdbutton.setSelected(true);
        // ボタンの表示設定
        url_rdbutton.setContentAreaFilled(false);
        url_rdbutton.setBorderPainted(false);
        url_rdbutton.setFocusPainted(false);
        // ボタンを押した際の処理
        url_rdbutton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                System.out.println("no_button pressed");
                if (url_rdbutton.isSelected()) { // 選択されているとき
                    // 入力フィールドに書き込みを行える状態にする
        ...
        // ボタンのアイコンの設定
        ImageIcon preaddb_icon = new ImageIcon("image/component/home/anikatsudb/presentation/searcher/button/sch_button.png");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (getInputData(data)) { // 入力した情報に不備がないとき
                    ...
                    try {
                        ...
                        // キャラクターに報告させる
                        bc_func.charaSpeak("研究発表情報の検索に成功し", "報告");
                    } catch (SQLException se) {
                        // キャラクターに報告させる
                        bc_func.charaSpeak("研究発表情報の検索に失敗し", "報告");
        ...
        // 各列の列名をセットする
        String res_column_names[] = {"発表日", "部員番号", "ハンドルネーム", "タイトル", "URL"};
        ...
        // ボタンのアイコンの設定
        ImageIcon bkb_icon = new ImageIcon("image/component/home/anikatsudb/presentation/searcher/button/bk_button.png");
    ...
    private boolean getInputData(final String data[]) {
        ...
        if (pftmth_button.isSelected()) { // 完全一致検索のとき
            ...
            if (year_rdbutton.isSelected() && year.isEmpty()) {
                // キャラクターに注意させる
                bc_func.charaSpeak("発表年を入力し", "命令2");
                return false;
            }
            else if (title_rdbutton.isSelected() && title.isEmpty()) {
                // キャラクターに注意させる
                bc_func.charaSpeak("タイトルを入力し", "命令2");
                return false;
            }
            else if (url_rdbutton.isSelected() && url.isEmpty()) {
                // キャラクターに注意させる
                bc_func.charaSpeak("URLを入力し", "命令2");
                return false;
            }
        }

        if (year_rdbutton.isSelected()) { // 発表年が検索条件の一つのとき
            if (!Pattern.compile("^-?[0-9]+$").matcher(year).find()) { // 数字出ないものが発表年の欄に含まれるとき。正規表現を使って発表年が数字であるかを確認している
                // キャラクターに注意させる
                bc_func.charaSpeak("発表年には、数字を入力し", "命令2");
                return false;
            }
            else if (year.length() != 4) { // 発表年の桁数が4桁ではないとき
                // キャラクターに注意させる
        ...
        if (title_rdbutton.isSelected() && title.length() > 30) { // タイトルが31文字以上のとき
            // キャラクターに注意させる
            bc_func.charaSpeak("タイトルは、30文字以内で入力し", "命令2");
            return false;
        }
        else if (url_rdbutton.isSelected() && url.length() > 70) { // URLが71文字以上のとき
            // キャラクターに注意させる
            ...
            if (year_rdbutton.isSelected()) { // 発表年を検索対象とするとき
                data[1] = year;
            }
            else { // 発表年を検索対象としないとき
                data[1] = "";
            ...
            if (title_rdbutton.isSelected()) { // タイトルを検索対象とするとき
                data[2] = title;
            }
            else { // タイトルを検索対象としないとき
                data[2] = "";
            }

            if (url_rdbutton.isSelected()) { // URLを検索対象とするとき
                data[3] = url;
            }
            else { // URLを検索対象としないとき
                data[3] = "";
    ...
    private ResultSet executeSearchSQL(final Statement stmt, final String data[]) throws SQLException {
        ...
        if (data[0].isEmpty() && data[1].isEmpty() && data[2].isEmpty() && data[3].isEmpty()) { // 検索条件がないとき
            rs = stmt.executeQuery("SELECT presentations.*, h_name FROM presentations JOIN members ON id = member_id");
        }
        else { // 検索条件が設定されているとき
            if (pftmth_button.isSelected()) { // 完全一致検索のとき
    ...
    private ResultSet executePftSearch(final Statement stmt, final String data[]) throws SQLException {
        ...
        if (data[1].isEmpty()) {
            ...
            String start_date = data[1] + "-01-01";
            String end_date = data[1] + "-12-31";
            data_stmt[1] = " presentation_date BETWEEN '" + start_date + "' AND '" + end_date + "'";
        }
        ...
        // 検索をし、検索結果を格納する
        ResultSet rs = stmt.executeQuery("SELECT presentations.*, h_name FROM presentations JOIN members ON id = member_id WHERE"
